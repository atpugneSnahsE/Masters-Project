...
def generate_xai_figure(rgb_image, frame_id):
    ...
    # Clean Ego Lane Mask
    roi_start = int(h * 0.55)
    roi = prob_vis[roi_start:, :]
    thresh = max(0.22, np.percentile(roi, 60))
    binary = (prob_vis > thresh).astype(np.uint8) * 255
    ...
    if n_labels > 1:
        ...
        ego_lane = (labels == largest).astype(np.uint8) * 255
    else:
        ego_lane = binary

    # Guide LIME
    enhanced = rgb_image.copy()
    enhanced[ego_lane > 0] = [80, 255, 140]

    print(f"   → Running LIME on frame {frame_id} ...")
    explainer = lime_image.LimeImageExplainer(verbose=False)
    explanation = explainer.explain_instance(
        enhanced.astype(np.double) / 255.0,
        batch_predict,
        top_labels=1,
        hide_color=0,
        num_features=30,
    ...
    # Trust Map
    confidence = (ego_lane > 80).astype(np.float32)
    trust_map = cv2.GaussianBlur(confidence, (31, 31), 0)
    ...
    axs[1].imshow(ego_lane, cmap='gray')
    axs[1].set_title("Detected Lane Area")
    axs[1].axis('off')

    axs[2].imshow(mark_boundaries(temp, lime_mask))
    axs[2].set_title("LIME Attribution\n(Lane Contribution)")
    axs[2].axis('off')

    im = axs[3].imshow(trust_map, cmap='RdYlGn', vmin=0, vmax=1)
    axs[3].set_title("Trust Score Map\n(High = Reliable)")
    axs[3].axis('off')
    plt.colorbar(im, ax=axs[3], fraction=0.046, pad=0.04)

    plt.suptitle(f"XAI Analysis - Frame {frame_id}", fontsize=18, y=1.02)
    plt.tight_layout()
...
latest = None
frame_count = 0
XAI_EVERY = 45  # Generate XAI every ~1.5 seconds
...
def classify_boundary(side, hist, side_id):
    ...
    if peak_val < 5:
        ...
        is_double_candidate = (second_val >= peak_val * 0.55 and 8 <= distance <= int(w * 0.11))
...
# ====================== PROCESS CALLBACK ======================
def process(image):
    global latest, frame_count, freeze_count, prev_left, prev_right, last_gap, last_trend, current_state
    ...
    prob = torch.sigmoid(pred).cpu().squeeze().numpy()
    lower_half = prob[int(prob.shape[0]*0.45):, :]
    thresh = max(0.36, np.percentile(lower_half, 89))
...
# ====================== MAIN LOOP ======================
print("🚀 CARLA + Advanced Lane Understanding + XAI Running")
...
try:
    while True:
        if latest is not None:
            ...
            cv2.imshow("EU Lane Understanding - Advanced + XAI", overlay)
