"""
VIL-100 Lane Detection — Fine-tune with full network (encoder unfrozen)
Run this AFTER train_vil100.py has finished.

Loads best_model.pth and continues training with:
  - Encoder unfrozen
  - Lower LR (5e-5) to avoid destroying learned weights
  - 20 more epochs

Usage:
    python3 finetune_vil100.py
"""
...
SAVE_DIR     = os.path.expanduser("/Users/mac/Downloads/vil100_model")
CHECKPOINT   = os.path.join(SAVE_DIR, "best_model.pth")  # load from stage 1
IMG_H        = 368
IMG_W        = 640
LANE_WIDTH   = 10
BATCH_SIZE   = 4
NUM_EPOCHS   = 20       # 20 more epochs with full network
LR           = 5e-5    # much lower — don't destroy encoder weights
VAL_SPLIT    = 0.1
BCE_WEIGHT   = 1.0
DICE_WEIGHT  = 3.0
DEVICE       = "cpu"
...
print("=" * 55)
print("  VIL-100 Fine-tuning (encoder unfrozen)")
print("=" * 55)
...
# ─────────────────────────────────────────────
# FILE LIST  (same as training script)
# ─────────────────────────────────────────────
    ...
    for clip in clip_dirs:
        ...
        for jf in sorted(os.listdir(clip_json_dir)):
            ...
            if os.path.exists(img_path):
                records.append({"image_path": img_path, "json_path": json_path})
    print(f"  Found {len(records)} frames across {len(clip_dirs)} clips")
    return records
...
class VIL100Dataset(Dataset):
    ...
    def _make_mask(self, json_path, img_h, img_w):
        ...
        for lane in data["annotations"].get("lane", []):
            points = lane.get("points", [])
            if len(points) < 2:
                continue
            scaled = [(int(p[0]*scale_x), int(p[1]*scale_y)) for p in points]
            for i in range(len(scaled)-1):
                cv2.line(mask, scaled[i], scaled[i+1], color=1, thickness=LANE_WIDTH)
        return mask
...
class DiceBCELoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        dice     = 1-(2*inter+self.smooth)/(probs.sum(dim=(1,2))+targets.sum(dim=(1,2))+self.smooth)
...
# ─────────────────────────────────────────────
# MODEL  —  load checkpoint, unfreeze everything
# ─────────────────────────────────────────────
model = smp.Unet(
    encoder_name    = "resnet34",
    encoder_weights = None,   # we load from checkpoint, not imagenet
    in_channels     = 3,
    classes         = 1,
)
...
print(f"  Trainable params : {trainable:,}  (full network)")
print("=" * 55)
...
print("=" * 55)
print(f"  Fine-tuning complete!  Best IoU : {best_iou:.4f}")
...
plt.suptitle("VIL-100 Fine-tuning — U-Net ResNet34 (full network)", fontsize=13)
plt.tight_layout()
curve_path = os.path.join(SAVE_DIR, "finetuning_curves.png")
